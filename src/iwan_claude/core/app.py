...
from iwan_claude.core.logging_setup import setup_logging
...
from iwan_claude.core.runs import events_file, new_run_id
from iwan_claude.core.session import SessionManager, SessionStore
...
class CoreApp:
    ...
    async def _ping_handler(self, params: dict[str, Any]) -> PongResult:
        ...
        return PongResult(
            server_version=iwan_claude.__version__,
    ...
    async def run(self) -> None:
        ...
        if not IS_WINDOWS:
            ...
            # Windows：默认 ProactorEventLoop 不支持 add_signal_handler → NotImplementedError
            # 退回到全局 signal.signal() + KeyboardInterrupt 兜底
            def _win_sigint_handler(signum: int, frame: object) -> None:  # pragma: no cover - windows only
                try:
                    loop.call_soon_threadsafe(_set_shutdown)
                except Exception:
                    # event loop 已停止的情况下，直接设置
                    _set_shutdown()
            signal.signal(signal.SIGINT, _win_sigint_handler)
            # 注意：Windows 没有 SIGTERM 信号，跳过

        try:
            await shutdown.wait()
        except KeyboardInterrupt:  # pragma: no cover - Ctrl+C 直接触发的兜底
            pass
