...
class AnthropicProvider:
    # 初始化 Anthropic 客户端（支持 DeepSeek 等原生兼容 Anthropic Messages API 的端点）
    #   - model: 模型名；DeepSeek 用 deepseek-chat / deepseek-reasoner，或直接传 claude-* 让后端自动映射
    #   - api_key_env: API Key 从哪个环境变量读（默认 ANTHROPIC_API_KEY）
    #   - base_url: 若传入，则覆盖默认的 api.anthropic.com；DeepSeek 填 https://api.deepseek.com/anthropic
    ...
    def __init__(
        ...
        if client is None:
            ...
            # 关键：传 base_url 到 SDK 构造器——DeepSeek 填了就能用原生 Anthropic Messages API
